...
void ROSHelpers::GetTransformFromTF(const std::string parent_frame,
    ...
    autoware_msgs::LaneArray& laneArray)
{
  ...
  for (unsigned int i = 0; i < path.size(); i++)
  {
    ...
    wp.pose.pose.orientation = tf::createQuaternionMsgFromYaw(
                  UtilityHNS::UtilityH::SplitPositiveAngle(path.at(i).pos.a));
...
void ROSHelpers::ConvertFromRoadNetworkToAutowareVisualizeMapFormat(const PlannerHNS::RoadNetwork& map,
...
void ROSHelpers::ConvertFromPlannerHToAutowareVisualizePathFormat(
      const std::vector<std::vector<PlannerHNS::WayPoint> >& globalPaths,
      visualization_msgs::MarkerArray& markerArray)
{
  visualization_msgs::Marker lane_waypoint_marker;
  lane_waypoint_marker.header.frame_id = "map";
                                  ...
                                  visualization_msgs::MarkerArray& markerArray)
{
  ...
  for (unsigned int i = 0; i < lane_waypoints_array.lanes.size(); i++)
  {
    ...
    for (unsigned int j = 0; j < lane_waypoints_array.lanes.at(i).waypoints.size(); j++)
    {
      ...
      relative_p.y = 0.5;
      velocity_marker.pose.position = calcAbsoluteCoordinate(relative_p,
                  lane_waypoints_array.lanes.at(i).waypoints.at(j).pose.pose);
...
void ROSHelpers::FindIncommingBranches(const std::vector<std::vector<PlannerHNS::WayPoint>>& globalPaths,
              const PlannerHNS::WayPoint& currPose, const double& min_distance,
              std::vector<PlannerHNS::WayPoint*>& branches, PlannerHNS::WayPoint* currOptions)
{
  ...
  if (globalPaths.size() > 0)
  {
    ...
    {
      d += hypot(globalPaths.at(0).at(i).pos.y - globalPaths.at(0).at(i-1).pos.y,
                  globalPaths.at(0).at(i).pos.x - globalPaths.at(0).at(i-1).pos.x);
      ...
      if ((d > min_distance) && (globalPaths.at(0).at(i).pFronts.size() > 1))
      {
        for (unsigned int j = 0; j < globalPaths.at(0).at(i).pFronts.size(); j++)
        {
          ...
          {
            if ((wp->actionCost.size() > 0) && (branches.at(ib)->actionCost.size() > 0)
                && (branches.at(ib)->actionCost.at(0).first == wp->actionCost.at(0).first))
            {
              bFound = true;
              break;
            }
